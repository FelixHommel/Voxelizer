...
Ray reconstructRay()
{
    ...
    r.invDirection = 1.0 / max(abs(r.direction), vec3(EPSILON)) * sign(r.direction);
...
/// \brief Calculate the normal of an voxel based on from which axis the voxel was hit by the ray.
///
/// \returns vec3 the normal of the voxel
vec3 calculateNormal(int hitAxis, vec3 voxelStep, Ray ray)
{
    if(hitAxis == HIT_AXIS_X)
        return vec3(-voxelStep.x, 0, 0);

    if(hitAxis == HIT_AXIS_Y)
        return vec3(0, -voxelStep.y, 0);

    if(hitAxis == HIT_AXIS_Z)
        return vec3(0, 0, -voxelStep.z);
...
void main()
{
    ...
    int hitAxis = HIT_AXIS_UNDEFINED;
    ...
    for(int i = 0; i < 256; ++i)
    {
        ...
        if(voxelColor.r > 0.0)
        {
            vec3 normal = calculateNormal(hitAxis, voxelStep, ray);
            float diffuse = max(dot(normal, lightDir), 0.0);
            float lighting = ambientLightStrength + (1.0 - ambientLightStrength) * diffuse;

            FragColor = vec4(voxelColor.rgb * lighting, voxelColor.a);
            return;
        }
